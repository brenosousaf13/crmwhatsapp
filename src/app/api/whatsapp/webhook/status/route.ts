...
export async function GET() {
    ...
    try {
        ...
        if (!response.ok) {
            return NextResponse.json({ error: 'Falha ao buscar status do webhook na API do WhatsApp' }, { status: response.status })
        }
        ...
        return NextResponse.json({ error: 'Erro interno ao buscar status do webhook' }, { status: 500 })
    }
}
